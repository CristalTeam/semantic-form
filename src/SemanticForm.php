...
class SemanticForm
{
    ...
    public function text($name, $defaultValue = null, $opt = [])
    {
        $text = new Text($name);
    ...
    public function date($name, $defaultValue = null)
    {
        ...
        if ($this->hasError($name)) {
            $date->setError($this->getError($name));
        }
    ...
    public function datepicker($name, $defaultValue = null)
    {
        ...
        if ($this->hasError($name)) {
            $input->setError($this->getError($name));
        }
    ...
    public function email($name, $defaultValue = null)
    {
        ...
        if ($this->hasError($name)) {
            $email->setError($this->getError($name));
        }
    ...
    public function textarea($name, $defaultValue = null, $opt = [])
    {
    ...
    public function password($name)
    {
        ...
        if ($this->hasError($name)) {
            $password->setError($this->getError($name));
        }

        return $password;
    }

    public function checkbox($name, $value = 1, $modifier = '', $opt = [])
    {
        $checkbox = new Checkbox($name, $value, $modifier);
    ...
    public function submit($value = 'Submit', $opt = [])
    {
    ...
    public function number($name, $defaultValue = null)
    {
        ...
        if ($this->hasError($name)) {
            $number->setError($this->getError($name));
        }
    ...
    // ça c'est nous
    public function input($name, $type = "text", ...$values)
    {
        $text = $this->{$type}($name, ...$values);
